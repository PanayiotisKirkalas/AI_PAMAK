...
#define MAX_TRIES 1000 // Maximum number of attempts for WalkSAT
#define PROB_FLIP 0.4  // Probability of flipping a variable in WalkSAT
...
// Function to select a random unsatisfied clause
int selectUnsatisfiedClause(int *vector)
{
    int* unsatisfiedClauses = (int*) malloc(M*sizeof(int));
    ...
    for (int i = 0; i < M; i++)
    {
        ...
        for (int j = 0; j < K; j++)
        {
            int proposition = Problem[i * K + j];
            if ((proposition > 0 && vector[proposition - 1] == 1) ||
                (proposition < 0 && vector[-proposition - 1] == -1))
            {
                satisfied = 1;
                break;
            }
        ...
        if (!satisfied)
        {
            unsatisfiedClauses[unsatisfiedCount++] = i;
        }
    }
    ...
    if (unsatisfiedCount > 0)
    {
        ...
        return unsatisfiedClauses[rand() % unsatisfiedCount];
    }
    else
    {
        // No unsatisfied clauses, return -1 as a flag
        return -1;
    }
}
...
void walkSAT(char **argv) {
    ...
    while (h > 0) {
        ...
        int unsatisfiedClause = selectUnsatisfiedClause(vector);

        steps++;

        if (unsatisfiedClause != -1) {
            // With a certain probability, flip a variable in the selected clause
            if ((float)rand() / RAND_MAX < PROB_FLIP) {
                int randomVariable = rand() % K;  // Select a random variable in the clause
                int variableIndex = abs(Problem[unsatisfiedClause * K + randomVariable]) - 1;
                vector[variableIndex] = -vector[variableIndex];  // Flip the variable
            } else {
                // Otherwise, perform hill climbing within the selected clause
                int h2, best_change = -1;
                for (i = 0; i < K; i++) {
                    int proposition = Problem[unsatisfiedClause * K + i];
                    int variableIndex = abs(proposition) - 1;
                    vector[variableIndex] = -vector[variableIndex];
                    h2 = count(vector);
                    if (h2 <= h) {
                        h = h2;
                        best_change = variableIndex;
                    }
                    vector[variableIndex] = -vector[variableIndex];
                }
            ...
            // No unsatisfied clauses, perform hill climbing as before
            int h2, best_change = -1;
            for (i = 0; i < N; i++) {
                vector[i] = -vector[i];
                h2 = count(vector);
